// Compiles a dart2wasm-generated main module from `source` which can then
// be instantiated via the `instantiate` method.
//
// `source` needs to be a `Response` object (or promise thereof) e.g. created
// via the `fetch()` JS API.
export async function compileStreaming(source) {
  const builtins = {builtins: ['js-string']};
  return new CompiledApp(
      await WebAssembly.compileStreaming(source, builtins), builtins);
}

// Compiles a dart2wasm-generated wasm module from `bytes` which is then
// instantiable via the `instantiate` method.
export async function compile(bytes) {
  const builtins = {builtins: ['js-string']};
  return new CompiledApp(await WebAssembly.compile(bytes, builtins), builtins);
}

class CompiledApp {
  constructor(module, builtins) {
    this.module = module;
    this.builtins = builtins;
  }

  // The second argument is an options object containing:
  // `loadDeferredModules` is a JS function that takes an array of module names
  //   matching wasm files produced by the dart2wasm compiler. It also takes a
  //   callback that should be invoked for each loaded module with 2 arguments:
  //   (1) the module name, (2) the loaded module in a format supported by
  //   `WebAssembly.compile` or `WebAssembly.compileStreaming`. The callback
  //   returns a Promise that resolves when the module is instantiated.
  //   loadDeferredModules should return a Promise that resolves when all the
  //   modules have been loaded and the callback promises have resolved.
  // `loadDeferredId` is a JS function that takes load ID produced by the
  //   compiler when the `use-load-ids` option is passed. Each load ID maps to
  //   one or more wasm files as specified in the emitted JSON file. It also
  //   takes a callback that should be invoked for each loaded module with 2
  //   arguments: (1) the module name, (2) the loaded module in a format
  //   supported by `WebAssembly.compile` or `WebAssembly.compileStreaming`.
  //   The callback returns a Promise that resolves when the module is
  //   instantiated.
  //   loadDeferredId should return a Promise that resolves when all the
  //   modules have been loaded and the callback promises have resolved.
  async instantiate(additionalImports, {loadDeferredModules, loadDeferredId} = {}) {
    let dartInstance;

    // Prints to the console
    function printToConsole(value) {
      if (typeof dartPrint == "function") {
        dartPrint(value);
        return;
      }
      if (typeof console == "object" && typeof console.log != "undefined") {
        console.log(value);
        return;
      }
      if (typeof print == "function") {
        print(value);
        return;
      }

      throw "Unable to print message: " + value;
    }

    // A special symbol attached to functions that wrap Dart functions.
    const jsWrappedDartFunctionSymbol = Symbol("JSWrappedDartFunction");

    function finalizeWrapper(dartFunction, wrapped) {
      wrapped.dartFunction = dartFunction;
      wrapped[jsWrappedDartFunctionSymbol] = true;
      return wrapped;
    }

    // Imports
    const dart2wasm = {
            AB: x0 => new Int16Array(x0),
      AC: (o, start, length) => new Uint8ClampedArray(o.buffer, o.byteOffset + start, length),
      AD: x0 => x0.screen,
      AE: x0 => new ResizeObserver(x0),
      AF: x0 => x0.key,
      AG: x0 => x0.pathname,
      AH: (x0,x1,x2,x3) => x0.initEvent(x1,x2,x3),
      AI: x0 => x0.offsetWidth,
      AJ: (x0,x1) => x0.postMessage(x1),
      AK: (x0,x1,x2,x3,x4) => x0.sqlite3_result_blob64(x1,x2,x3,x4),
      AL: (x0,x1,x2) => x0.write(x1,x2),
      AM: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2,x3,x4) { return wasmFunction(f,arguments.length,x0,x1,x2,x3,x4) }),
      AN: (x0,x1) => { x0.height = x1 },
      AO: x0 => x0.response,
      AP: (x0,x1) => x0.key(x1),
      AQ: x0 => x0.stopContinuousDecode,
      AR: x0 => x0.topLeft,
      AS: x0 => x0.length,
      B: s => printToConsole(s),
      BB: (jsArray, jsArrayOffset, wasmArray, wasmArrayOffset, length) => {
        const getValue = dartInstance.exports.$wasmI16ArrayGet;
        for (let i = 0; i < length; i++) {
          jsArray[jsArrayOffset + i] = getValue(wasmArray, wasmArrayOffset + i);
        }
      },
      BC: (o, start, length) => new Uint8Array(o.buffer, o.byteOffset + start, length),
      BD: o => {
        if (o === null || o === undefined) return 0;
        if (typeof(o) === 'string') return 1;
        return 2;
      },
      BE: (x0,x1) => x0.getPropertyValue(x1),
      BF: x0 => x0.identifier,
      BG: x0 => x0.parentElement,
      BH: x0 => x0.readText(),
      BI: x0 => x0.stopPropagation(),
      BJ: (x0,x1) => ({kind: x0,table: x1}),
      BK: (x0,x1,x2,x3,x4) => x0.sqlite3_result_text(x1,x2,x3,x4),
      BL: x0 => x0.createSyncAccessHandle(),
      BM: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      BN: (x0,x1) => { x0.width = x1 },
      BO: (x0,x1) => { x0.responseType = x1 },
      BP: x0 => x0.length,
      BQ: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      BR: x0 => x0.position,
      BS: x0 => x0.getReader(),
      C: Function.prototype.call.bind(Number.prototype.toString),
      CB: x0 => new Uint16Array(x0),
      CC: (o, start, length) => new Int8Array(o.buffer, o.byteOffset + start, length),
      CD: x0 => x0.tabIndex,
      CE: x0 => globalThis.parseFloat(x0),
      CF: x0 => x0.touches,
      CG: (x0,x1) => x0.querySelectorAll(x1),
      CH: x0 => x0.clipboard,
      CI: x0 => x0.disabled,
      CJ: (o, p) => p in o,
      CK: (x0,x1,x2) => x0.sqlite3_result_double(x1,x2),
      CL: x0 => ({create: x0}),
      CM: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      CN: x0 => x0.style,
      CO: x0 => x0.vendor,
      CP: (x0,x1) => x0.canShare(x1),
      CQ: (x0,x1,x2,x3) => x0.call(x1,x2,x3),
      CR: x0 => x0.isValid,
      CS: x0 => x0.value,
      D: Function.prototype.call.bind(BigInt.prototype.toString),
      DB: x0 => new Int32Array(x0),
      DC: (x0,x1) => x0.querySelector(x1),
      DD: (x0,x1) => x0.contains(x1),
      DE: (x0,x1) => x0.getComputedStyle(x1),
      DF: x0 => x0.pressure,
      DG: (x0,x1) => x0.requestAnimationFrame(x1),
      DH: (x0,x1) => x0.writeText(x1),
      DI: (x0,x1) => { x0.min = x1 },
      DJ: x0 => x0.groups,
      DK: (x0,x1,x2) => x0.sqlite3_result_int64(x1,x2),
      DL: (x0,x1,x2) => x0.getFileHandle(x1,x2),
      DM: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2) { return wasmFunction(f,arguments.length,x0,x1,x2) }),
      DN: (x0,x1) => { x0.src = x1 },
      DO: x0 => x0.navigator,
      DP: (x0,x1) => x0.share(x1),
      DQ: x0 => x0.text,
      DR: (x0,x1,x2,x3) => ({formats: x0,tryHarder: x1,tryRotate: x2,tryInvert: x3}),
      DS: x0 => x0.done,
      E: (exn) => {
        let stackString = exn.toString();
        let frames = stackString.split('\n');
        let drop = 4;
        if (frames[0].startsWith('Error')) {
            drop += 1;
        }
        return frames.slice(drop).join('\n');
      },
      EB: (jsArray, jsArrayOffset, wasmArray, wasmArrayOffset, length) => {
        const getValue = dartInstance.exports.$wasmI32ArrayGet;
        for (let i = 0; i < length; i++) {
          jsArray[jsArrayOffset + i] = getValue(wasmArray, wasmArrayOffset + i);
        }
      },
      EC: (x0,x1) => x0.item(x1),
      ED: x0 => x0.activeElement,
      EE: x0 => x0.documentElement,
      EF: x0 => x0.tiltY,
      EG: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      EH: x0 => x0.unlock(),
      EI: (x0,x1) => { x0.max = x1 },
      EJ: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      EK: (x0,x1) => x0.sqlite3_result_null(x1),
      EL: x0 => ({create: x0}),
      EM: (x0,x1) => x0.read(x1),
      EN: () => globalThis.document,
      EO: () => globalThis.window,
      EP: x0 => x0.message,
      EQ: x0 => x0.barcodeFormat,
      ER: (x0,x1,x2) => ({tryHarder: x0,tryRotate: x1,tryInvert: x2}),
      ES: x0 => x0.read(),
      F: () => new Error().stack,
      FB: x0 => new Uint32Array(x0),
      FC: x0 => x0.length,
      FD: x0 => x0.parentNode,
      FE: x0 => x0.computedStyleMap(),
      FF: x0 => x0.tiltX,
      FG: x0 => x0.now(),
      FH: (x0,x1) => x0.lock(x1),
      FI: (x0,x1) => { x0.disabled = x1 },
      FJ: (x0,x1) => x0.postMessage(x1),
      FK: (x0,x1) => x0.sqlite3_value_blob(x1),
      FL: (x0,x1,x2) => x0.getDirectoryHandle(x1,x2),
      FM: (x0,x1,x2) => x0.read(x1,x2),
      FN: x0 => x0.src,
      FO: (x0,x1) => x0.get(x1),
      FP: (x0,x1) => ({files: x0,text: x1}),
      FQ: x0 => x0.rawBytes,
      FR: () => globalThis.ZXingWASM,
      FS: (x0,x1) => new OffscreenCanvas(x0,x1),
      G: s => JSON.stringify(s),
      GB: x0 => new Float32Array(x0),
      GC: (x0,x1) => x0.querySelectorAll(x1),
      GD: x0 => x0.tagName,
      GE: (x0,x1) => x0.get(x1),
      GF: x0 => x0.pointerType,
      GG: x0 => x0.performance,
      GH: x0 => x0.orientation,
      GI: (x0,x1) => { x0.scrollLeft = x1 },
      GJ: x0 => x0.close(),
      GK: (x0,x1) => x0.sqlite3_value_bytes(x1),
      GL: (x0,x1) => new URL(x0,x1),
      GM: x0 => x0.flush(),
      GN: (x0,x1) => x0.revokeObjectURL(x1),
      GO: x0 => x0.body,
      GP: x0 => ({files: x0}),
      GQ: x0 => x0.y,
      GR: (x0,x1) => { x0.height = x1 },
      GS: x0 => x0.assetBase,
      H: Function.prototype.call.bind(Number.prototype.toString),
      HB: (jsArray, jsArrayOffset, wasmArray, wasmArrayOffset, length) => {
        const getValue = dartInstance.exports.$wasmF32ArrayGet;
        for (let i = 0; i < length; i++) {
          jsArray[jsArrayOffset + i] = getValue(wasmArray, wasmArrayOffset + i);
        }
      },
      HC: (x0,x1) => x0.getAttribute(x1),
      HD: x0 => x0.target,
      HE: (o, p) => p in o,
      HF: x0 => x0.pointerId,
      HG: (d, digits) => d.toFixed(digits),
      HH: (x0,x1) => x0.querySelector(x1),
      HI: (x0,x1) => { x0.spellcheck = x1 },
      HJ: (x0,x1) => ({i: x0,p: x1}),
      HK: (x0,x1) => x0.sqlite3_value_text(x1),
      HL: x0 => x0.pathname,
      HM: () => globalThis.WebAssembly,
      HN: (x0,x1) => { x0.src = x1 },
      HO: x0 => x0.headers,
      HP: x0 => ({text: x0}),
      HQ: x0 => x0.x,
      HR: (x0,x1) => { x0.width = x1 },
      HS: x0 => x0.loader,
      I: Function.prototype.call.bind(String.prototype.indexOf),
      IB: x0 => new Float64Array(x0),
      IC: x0 => x0.remove(),
      ID: x0 => x0.clientY,
      IE: (x0,x1) => { x0.textContent = x1 },
      IF: x0 => x0.getCoalescedEvents(),
      IG: x0 => x0.maxHeight,
      IH: (x0,x1) => { x0.title = x1 },
      II: (x0,x1) => { x0.disabled = x1 },
      IJ: () => new Array(),
      IK: (x0,x1) => x0.sqlite3_value_double(x1),
      IL: x0 => x0.getDirectory(),
      IM: x0 => x0.href,
      IN: (x0,x1,x2,x3,x4) => globalThis.createImageBitmap(x0,x1,x2,x3,x4),
      IO: (x0,x1) => x0.getItem(x1),
      IP: (x0,x1) => { x0.transform = x1 },
      IQ: x0 => x0.resultPoints,
      IR: x0 => x0.height,
      IS: () => globalThis._flutter,
      J: (s, p, i) => s.lastIndexOf(p, i),
      JB: (jsArray, jsArrayOffset, wasmArray, wasmArrayOffset, length) => {
        const getValue = dartInstance.exports.$wasmF64ArrayGet;
        for (let i = 0; i < length; i++) {
          jsArray[jsArrayOffset + i] = getValue(wasmArray, wasmArrayOffset + i);
        }
      },
      JC: (x0,x1) => x0.appendChild(x1),
      JD: x0 => x0.clientX,
      JE: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      JF: (x0,x1) => x0.getModifierState(x1),
      JG: x0 => x0.maxWidth,
      JH: (x0,x1) => x0.vibrate(x1),
      JI: (x0,x1) => x0.getRandomValues(x1),
      JJ: (x0,x1) => ({c: x0,r: x1}),
      JK: (x0,x1) => x0.sqlite3_value_int64(x1),
      JL: x0 => x0.storage,
      JM: (x0,x1) => x0.openCursor(x1),
      JN: x0 => x0.naturalHeight,
      JO: x0 => x0.localStorage,
      JP: x0 => x0.style,
      JQ: x0 => x0.message,
      JR: x0 => x0.width,
      K: (exn) => {
        if (exn instanceof Error) {
          return exn.stack;
        } else {
          return null;
        }
      },
      KB: x0 => new ArrayBuffer(x0),
      KC: (x0,x1) => x0.append(x1),
      KD: (x0,x1,x2) => x0.setAttribute(x1,x2),
      KE: x0 => x0.matches,
      KF: s => s.trimLeft(),
      KG: x0 => x0.minHeight,
      KH: x0 => x0.arrayBuffer(),
      KI: () => globalThis.crypto,
      KJ: (x0,x1) => { x0.onmessage = x1 },
      KK: (x0,x1) => x0.sqlite3_value_type(x1),
      KL: () => globalThis.navigator,
      KM: x0 => x0.arrayBuffer(),
      KN: x0 => x0.naturalWidth,
      KO: (x0,x1,x2,x3) => x0.replaceState(x1,x2,x3),
      KP: x0 => x0.getVideoTracks(),
      KQ: x0 => x0.videoElement,
      KR: (x0,x1) => { x0.srcObject = x1 },
      L: o => o === undefined,
      LB: (x0,x1,x2) => new Uint8Array(x0,x1,x2),
      LC: (x0,x1,x2,x3) => x0.setProperty(x1,x2,x3),
      LD: x0 => x0.getBoundingClientRect(),
      LE: (x0,x1) => x0.matchMedia(x1),
      LF: (x0,x1) => x0[x1],
      LG: x0 => x0.minWidth,
      LH: o => {
        if (o === null || o === undefined) return 0;
        if (o instanceof ArrayBuffer) return 1;
        if (globalThis.SharedArrayBuffer !== undefined &&
            o instanceof SharedArrayBuffer) {
          return 2;
        }
        return 3;
      },
      LI: l => new DataView(new ArrayBuffer(l)),
      LJ: (o, a) => o == a,
      LK: (x0,x1,x2) => x0.sqlite3_extended_result_codes(x1,x2),
      LL: (x0,x1) => globalThis.fetch(x0,x1),
      LM: x0 => x0.value,
      LN: x0 => x0.decode(),
      LO: x0 => x0.history,
      LP: x0 => x0.getSettings(),
      LQ: x0 => x0.decodeContinuously,
      LR: x0 => ({willReadFrequently: x0}),
      M: o => String(o),
      MB: (x0,x1,x2) => new DataView(x0,x1,x2),
      MC: x0 => x0.style,
      MD: (ms, c) =>
      setTimeout(() => dartInstance.exports.$invokeCallback(c),ms),
      ME: x0 => x0.matches,
      MF: x0 => x0.index,
      MG: (x0,x1) => x0.removeProperty(x1),
      MH: x0 => x0.status,
      MI: (a, i) => a.splice(i, 1),
      MJ: (o, t) => typeof o === t,
      MK: (x0,x1,x2,x3,x4) => x0.sqlite3_open_v2(x1,x2,x3,x4),
      ML: (x0,x1) => x0.sqlite3session_delete(x1),
      MM: x0 => x0.key,
      MN: (x0,x1) => { x0.decoding = x1 },
      MO: x0 => x0.href,
      MP: x0 => x0.facingMode,
      MQ: (x0,x1) => new ZXing.BrowserMultiFormatReader(x0,x1),
      MR: (x0,x1,x2) => x0.getContext(x1,x2),
      N: (c) =>
      queueMicrotask(() => dartInstance.exports.$invokeCallback(c)),
      NB: (o, p) => o[p],
      NC: x0 => x0.debugShowSemanticsNodes,
      ND: s => new Date(s * 1000).getTimezoneOffset() * 60,
      NE: o => typeof o === 'function' && o[jsWrappedDartFunctionSymbol] === true,
      NF: s => s.toUpperCase(),
      NG: (x0,x1) => x0.add(x1),
      NH: (x0,x1) => x0.fetch(x1),
      NI: a => a.pop(),
      NJ: x0 => x0.r,
      NK: x0 => x0.sqlite3_initialize(),
      NL: (x0,x1,x2,x3) => x0.register(x1,x2,x3),
      NM: x0 => x0.continue(),
      NN: (x0,x1) => { x0.crossOrigin = x1 },
      NO: x0 => x0.location,
      NP: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      NQ: (x0,x1) => ({width: x0,height: x1}),
      NR: () => new BarcodeDetector(),
      O: (x0,x1) => x0.didCreateEngineInitializer(x1),
      OB: (o) => new DataView(o.buffer, o.byteOffset, o.byteLength),
      OC: o => o,
      OD: Date.now,
      OE: f => f.dartFunction,
      OF: x0 => x0.pop(),
      OG: x0 => x0.data,
      OH: x0 => x0.content,
      OI: (map, o, v) => map.set(o, v),
      OJ: x0 => x0.c,
      OK: (x0,x1,x2,x3) => x0.dart_sqlite3_register_vfs(x1,x2,x3),
      OL: (x0,x1) => x0.unregister(x1),
      OM: x0 => x0.error,
      ON: (x0,x1) => x0.createObjectURL(x1),
      OO: (x0,x1) => x0.removeItem(x1),
      OP: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      OQ: (x0,x1,x2) => ({width: x0,height: x1,facingMode: x2}),
      OR: x0 => ({formats: x0}),
      P: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      PB: Function.prototype.call.bind(Object.getOwnPropertyDescriptor(DataView.prototype, 'byteLength').get),
      PC: o => {
        if (o === undefined || o === null) return 0;
        if (typeof o === 'boolean') return 1;
        return 2;
      },
      PD: (handle) => clearTimeout(handle),
      PE: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      PF: x0 => x0.flags,
      PG: (x0,x1) => { x0.scrollTop = x1 },
      PH: x0 => x0.document,
      PI: (map, o) => map.get(o),
      PJ: x0 => x0.p,
      PK: (x0,x1,x2) => x0.transaction(x1,x2),
      PL: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      PM: x0 => x0.result,
      PN: x0 => x0.URL,
      PO: (x0,x1,x2) => x0.setItem(x1,x2),
      PP: (x0,x1) => x0.append(x1),
      PQ: x0 => x0.facingMode,
      PR: x0 => new BarcodeDetector(x0),
      Q: (wasmFunction,f) => finalizeWrapper(f, function() { return wasmFunction(f,arguments.length) }),
      QB: o => o.byteOffset,
      QC: (x0,x1) => x0.warn(x1),
      QD: (x0,x1) => x0.closest(x1),
      QE: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      QF: (a, s) => a.join(s),
      QG: (x0,x1,x2) => x0.setSelectionRange(x1,x2),
      QH: () => typeof dartUseDateNowForTicks !== "undefined",
      QI: () => new WeakMap(),
      QJ: x0 => x0.i,
      QK: x0 => x0.close(),
      QL: x0 => new FinalizationRegistry(x0),
      QM: (x0,x1) => globalThis.IDBKeyRange.bound(x0,x1),
      QN: x0 => new Blob(x0),
      QO: (x0,x1) => new WebSocket(x0,x1),
      QP: (x0,x1) => x0.createElement(x1),
      QQ: x0 => x0.height,
      QR: (x0,x1) => x0.detect(x1),
      R: (x0,x1) => ({initializeEngine: x0,autoStart: x1}),
      RB: o => o.buffer,
      RC: x0 => x0.console,
      RD: x0 => x0.bottom,
      RE: (p, s, f) => p.then(s, (e) => f(e, e === undefined)),
      RF: (x0,x1) => x0.error(x1),
      RG: (x0,x1) => { x0.value = x1 },
      RH: () => Date.now(),
      RI: x0 => new WeakRef(x0),
      RJ: x0 => x0.port1,
      RK: (wasmFunction,f) => finalizeWrapper(f, function() { return wasmFunction(f,arguments.length) }),
      RL: () => globalThis.FinalizationRegistry,
      RM: x0 => x0.length,
      RN: x0 => x0.close(),
      RO: x0 => x0.reason,
      RP: () => globalThis.document,
      RQ: x0 => x0.width,
      RR: x0 => x0.rawValue,
      S: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      SB: Function.prototype.call.bind(DataView.prototype.getUint8),
      SC: () => globalThis.window,
      SD: x0 => x0.top,
      SE: (o, i) => o[i],
      SF: () => globalThis.console,
      SG: (x0,x1,x2) => x0.setSelectionRange(x1,x2),
      SH: () => 1000 * performance.now(),
      SI: x0 => x0.deref(),
      SJ: (x0,x1) => x0.sqlite3_changes(x1),
      SK: () => globalThis.Promise.resolve(),
      SL: (x0,x1) => x0.sqlite3changeset_finalize(x1),
      SM: (x0,x1) => x0.get(x1),
      SN: (x0,x1) => ({frameIndex: x0,completeFramesOnly: x1}),
      SO: x0 => x0.code,
      SP: (x0,x1) => { x0.onpause = x1 },
      SQ: x0 => x0.attachStreamToVideo,
      SR: x0 => x0.format,
      T: x0 => new Promise(x0),
      TB: (b, o) => new DataView(b, o),
      TC: (o, c) => o instanceof c,
      TD: x0 => x0.right,
      TE: o => o.length,
      TF: s => s.trimRight(),
      TG: (x0,x1) => { x0.value = x1 },
      TH: x0 => new Uint8Array(x0),
      TI: () => globalThis.WeakRef,
      TJ: (x0,x1) => x0.sqlite3_finalize(x1),
      TK: (x0,x1) => x0.then(x1),
      TL: x0 => x0.exports,
      TM: (x0,x1) => x0.index(x1),
      TN: (x0,x1) => x0.decode(x1),
      TO: (x0,x1,x2) => x0.close(x1,x2),
      TP: (x0,x1) => { x0.onplay = x1 },
      TQ: () => new Map(),
      TR: x0 => x0.y,
      U: (x0,x1,x2) => x0.call(x1,x2),
      UB: (b, o, l) => new DataView(b, o, l),
      UC: (x0,x1) => x0.exec(x1),
      UD: x0 => x0.left,
      UE: o => {
        if (o === undefined) return 1;
        var type = typeof o;
        if (type === 'boolean') return 2;
        if (type === 'number') return 3;
        if (type === 'string') return 4;
        if (o instanceof Array) return 5;
        if (ArrayBuffer.isView(o)) {
          if (o instanceof Int8Array) return 6;
          if (o instanceof Uint8Array) return 7;
          if (o instanceof Uint8ClampedArray) return 8;
          if (o instanceof Int16Array) return 9;
          if (o instanceof Uint16Array) return 10;
          if (o instanceof Int32Array) return 11;
          if (o instanceof Uint32Array) return 12;
          if (o instanceof Float32Array) return 13;
          if (o instanceof Float64Array) return 14;
          if (o instanceof DataView) return 15;
        }
        if (o instanceof ArrayBuffer) return 16;
        // Feature check for `SharedArrayBuffer` before doing a type-check.
        if (globalThis.SharedArrayBuffer !== undefined &&
            o instanceof SharedArrayBuffer) {
            return 17;
        }
        if (o instanceof Promise) return 18;
        return 19;
      },
      UF: x0 => x0.blur(),
      UG: s => {
        if (/[[\]{}()*+?.\\^$|]/.test(s)) {
            s = s.replace(/[[\]{}()*+?.\\^$|]/g, '\\$&');
        }
        return s;
      },
      UH: (x0,x1,x2) => x0.slice(x1,x2),
      UI: (o, offsetInBytes, lengthInBytes) => {
        var dst = new ArrayBuffer(lengthInBytes);
        new Uint8Array(dst).set(new Uint8Array(o, offsetInBytes, lengthInBytes));
        return new DataView(dst);
      },
      UJ: (x0,x1) => x0.sqlite3_reset(x1),
      UK: x0 => x0.abort(),
      UL: x0 => x0.call(),
      UM: x0 => x0.openKeyCursor(),
      UN: x0 => x0.displayHeight,
      UO: (x0,x1) => x0.close(x1),
      UP: (x0,x1) => { x0.controls = x1 },
      UQ: (x0,x1,x2) => x0.set(x1,x2),
      UR: x0 => x0.x,
      V: (constructor, args) => {
        const factoryFunction = constructor.bind.apply(
            constructor, [null, ...args]);
        return new factoryFunction();
      },
      VB: Function.prototype.call.bind(DataView.prototype.getFloat64),
      VC: x0 => x0.length,
      VD: x0 => x0.clientY,
      VE: x0 => x0.language,
      VF: x0 => x0.button,
      VG: x0 => x0.value,
      VH: (x0,x1) => x0.decode(x1),
      VI: (a, s, e) => a.slice(s, e),
      VJ: x0 => x0.buffer,
      VK: x0 => x0.commit(),
      VL: x0 => x0.instance,
      VM: x0 => x0.primaryKey,
      VN: x0 => x0.displayWidth,
      VO: x0 => x0.close(),
      VP: (x0,x1) => { x0.pointerEvents = x1 },
      VQ: (x0,x1) => x0.querySelector(x1),
      VR: x0 => x0.cornerPoints,
      W: x0 => new Array(x0),
      WB: o => {
        if (o === null || o === undefined) return 0;
        if (o instanceof Float64Array) return 1;
        return 2;
      },
      WC: (x0,x1) => { x0.lastIndex = x1 },
      WD: x0 => x0.clientX,
      WE: (x0,x1,x2,x3) => x0.register(x1,x2,x3),
      WF: x0 => x0.innerHeight,
      WG: x0 => x0.selectionDirection,
      WH: (x0,x1) => x0.adoptText(x1),
      WI: () => {
        return typeof process != "undefined" &&
               Object.prototype.toString.call(process) == "[object process]" &&
               process.platform == "win32"
      },
      WJ: (x0,x1) => x0.sqlite3_errstr(x1),
      WK: (wasmFunction,f) => finalizeWrapper(f, function() { return wasmFunction(f,arguments.length) }),
      WL: (x0,x1,x2) => x0.instantiateStreaming(x1,x2),
      WM: (x0,x1,x2) => x0.open(x1,x2),
      WN: x0 => x0.duration,
      WO: (x0,x1) => x0.send(x1),
      WP: (x0,x1) => { x0.transformOrigin = x1 },
      WQ: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      WR: x0 => x0.body,
      X: o => [o],
      XB: Function.prototype.call.bind(DataView.prototype.setFloat64),
      XC: (s, m) => {
        try {
          return new RegExp(s, m);
        } catch (e) {
          return String(e);
        }
      },
      XD: x0 => x0.changedTouches,
      XE: () => globalThis.window.FinalizationRegistry,
      XF: x0 => x0.innerWidth,
      XG: x0 => x0.selectionStart,
      XH: x0 => x0.first(),
      XI: () => {
        // On browsers return `globalThis.location.href`
        if (globalThis.location != null) {
          return globalThis.location.href;
        }
        return null;
      },
      XJ: (x0,x1) => x0.sqlite3_errmsg(x1),
      XK: (wasmFunction,f) => finalizeWrapper(f, function() { return wasmFunction(f,arguments.length) }),
      XL: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      XM: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      XN: x0 => x0.image,
      XO: x0 => x0.readyState,
      XP: (x0,x1) => { x0.objectFit = x1 },
      XQ: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      XR: x0 => globalThis.URL.revokeObjectURL(x0),
      Y: (o0, o1) => [o0, o1],
      YB: (t, s) => t.set(s),
      YC: o => o instanceof RegExp,
      YD: x0 => x0.offsetY,
      YE: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      YF: x0 => x0.height,
      YG: x0 => x0.selectionEnd,
      YH: x0 => x0.next(),
      YI: () => new AbortController(),
      YJ: (x0,x1) => x0.sqlite3_error_offset(x1),
      YK: (x0,x1) => { x0.onerror = x1 },
      YL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      YM: (x0,x1) => { x0.onupgradeneeded = x1 },
      YN: (x0,x1,x2,x3,x4) => ({type: x0,data: x1,premultiplyAlpha: x2,colorSpaceConversion: x3,preferAnimation: x4}),
      YO: (x0,x1) => { x0.binaryType = x1 },
      YP: (x0,x1) => { x0.width = x1 },
      YQ: (x0,x1) => x0.appendChild(x1),
      YR: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      Z: (o0, o1, o2) => [o0, o1, o2],
      ZB: Function.prototype.call.bind(DataView.prototype.setFloat32),
      ZC: (string, times) => string.repeat(times),
      ZD: x0 => x0.offsetX,
      ZE: x0 => new window.FinalizationRegistry(x0),
      ZF: x0 => x0.width,
      ZG: x0 => x0.value,
      ZH: x0 => x0.current(),
      ZI: (x0,x1,x2,x3,x4,x5) => ({method: x0,headers: x1,body: x2,credentials: x3,redirect: x4,signal: x5}),
      ZJ: (x0,x1) => x0.sqlite3_extended_errcode(x1),
      ZK: x0 => new DOMException(x0),
      ZL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2,x3,x4) { return wasmFunction(f,arguments.length,x0,x1,x2,x3,x4) }),
      ZM: x0 => ({autoIncrement: x0}),
      ZN: x0 => new window.ImageDecoder(x0),
      ZO: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      ZP: (x0,x1) => { x0.height = x1 },
      ZQ: x0 => x0.head,
      ZR: (x0,x1,x2,x3) => x0.toBlob(x1,x2,x3),
      a: (o0, o1, o2, o3) => [o0, o1, o2, o3],
      aB: Function.prototype.call.bind(DataView.prototype.getFloat32),
      aC: x0 => x0.dotAll,
      aD: x0 => x0.type,
      aE: (x0,x1) => x0.unregister(x1),
      aF: x0 => x0.clientHeight,
      aG: x0 => x0.selectionDirection,
      aH: (x0,x1) => new Intl.v8BreakIterator(x0,x1),
      aI: (x0,x1) => globalThis.fetch(x0,x1),
      aJ: (x0,x1) => x0.sqlite3_step(x1),
      aK: x0 => x0.error,
      aL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2) { return wasmFunction(f,arguments.length,x0,x1,x2) }),
      aM: (x0,x1,x2) => x0.createObjectStore(x1,x2),
      aN: x0 => x0.name,
      aO: (x0,x1) => { x0.onmessage = x1 },
      aP: x0 => x0.getSupportedConstraints(),
      aQ: (x0,x1) => { x0.onerror = x1 },
      aR: x0 => globalThis.URL.createObjectURL(x0),
      b: (x0,x1,x2) => { x0[x1] = x2 },
      bB: o => {
        if (o === null || o === undefined) return 0;
        if (o instanceof Float32Array) return 1;
        return 2;
      },
      bC: x0 => x0.unicode,
      bD: x0 => x0.maxTouchPoints,
      bE: (x0,x1) => x0.contains(x1),
      bF: x0 => x0.clientWidth,
      bG: x0 => x0.selectionStart,
      bH: x0 => x0.v8BreakIterator,
      bI: (x0,x1) => x0.get(x1),
      bJ: (x0,x1,x2,x3,x4) => x0.dart_sqlite3_bind_blob(x1,x2,x3,x4),
      bK: (x0,x1) => { x0.onabort = x1 },
      bL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2,x3) { return wasmFunction(f,arguments.length,x0,x1,x2,x3) }),
      bM: x0 => ({unique: x0}),
      bN: x0 => x0.repetitionCount,
      bO: x0 => x0.baseURI,
      bP: x0 => ({ideal: x0}),
      bQ: (x0,x1) => x0.removeChild(x1),
      bR: x0 => x0.size,
      c: o => o,
      cB: Function.prototype.call.bind(DataView.prototype.getUint32),
      cC: x0 => x0.ignoreCase,
      cD: x0 => x0.platform,
      cE: (s) => +s,
      cF: (x0,x1) => { x0.content = x1 },
      cG: x0 => x0.selectionEnd,
      cH: () => globalThis.Intl,
      cI: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2) { return wasmFunction(f,arguments.length,x0,x1,x2) }),
      cJ: (x0,x1) => x0.dart_sqlite3_malloc(x1),
      cK: (x0,x1) => { x0.oncomplete = x1 },
      cL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2,x3) { return wasmFunction(f,arguments.length,x0,x1,x2,x3) }),
      cM: (x0,x1,x2,x3) => x0.createIndex(x1,x2,x3),
      cN: x0 => x0.frameCount,
      cO: (x0,x1) => x0.transferFromImageBitmap(x1),
      cP: (x0,x1,x2) => ({width: x0,height: x1,deviceId: x2}),
      cQ: (x0,x1) => { x0.onload = x1 },
      cR: (x0,x1,x2,x3,x4,x5) => x0.drawImage(x1,x2,x3,x4,x5),
      d: (o, p) => o[p],
      dB: o => {
        if (o === null || o === undefined) return 0;
        if (o instanceof Uint32Array) return 1;
        return 2;
      },
      dC: x0 => x0.multiline,
      dD: x0 => x0.body,
      dE: s => {
        if (!/^\s*[+-]?(?:Infinity|NaN|(?:\.\d+|\d+(?:\.\d*)?)(?:[eE][+-]?\d+)?)\s*$/.test(s)) {
          return NaN;
        }
        return parseFloat(s);
      },
      dF: (x0,x1) => { x0.name = x1 },
      dG: x0 => x0.keyCode,
      dH: (x0,x1) => x0.segment(x1),
      dI: (x0,x1) => x0.forEach(x1),
      dJ: (x0,x1,x2,x3,x4) => x0.dart_sqlite3_bind_text(x1,x2,x3,x4),
      dK: (x0,x1) => x0.objectStore(x1),
      dL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2) { return wasmFunction(f,arguments.length,x0,x1,x2) }),
      dM: (x0,x1) => x0.createObjectStore(x1),
      dN: x0 => x0.selectedTrack,
      dO: (x0,x1) => x0.getContext(x1),
      dP: x0 => ({video: x0}),
      dQ: (x0,x1) => { x0.src = x1 },
      dR: (x0,x1) => x0.getContext(x1),
      e: () => globalThis,
      eB: Function.prototype.call.bind(DataView.prototype.getInt32),
      eC: (string, token) => string.split(token),
      eD: () => globalThis.document,
      eE: s => s.trim(),
      eF: x0 => x0.head,
      eG: (x0,x1) => x0.scrollIntoView(x1),
      eH: x0 => x0.index,
      eI: x0 => x0.name,
      eJ: (x0,x1,x2,x3) => x0.sqlite3_bind_double(x1,x2,x3),
      eK: (x0,x1) => globalThis.Atomics.load(x0,x1),
      eL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      eM: x0 => x0.oldVersion,
      eN: x0 => x0.completed,
      eO: (x0,x1) => { x0.height = x1 },
      eP: (x0,x1) => ({width: x0,height: x1}),
      eQ: (x0,x1) => { x0.crossOrigin = x1 },
      eR: x0 => x0.height,
      f: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      fB: o => {
        if (o === null || o === undefined) return 0;
        if (o instanceof Int32Array) return 1;
        return 2;
      },
      fC: o => o instanceof Array,
      fD: (x0,x1,x2) => x0.addEventListener(x1,x2),
      fE: x0 => x0.classList,
      fF: (x0,x1) => x0.removeChild(x1),
      fG: x0 => x0.multiViewEnabled,
      fH: x0 => x0.next(),
      fI: x0 => x0.statusText,
      fJ: (x0,x1,x2,x3) => x0.sqlite3_bind_int64(x1,x2,x3),
      fK: (x0,x1,x2) => globalThis.Atomics.wait(x0,x1,x2),
      fL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      fM: () => globalThis.indexedDB,
      fN: x0 => x0.ready,
      fO: (x0,x1) => { x0.width = x1 },
      fP: (x0,x1,x2) => ({width: x0,height: x1,facingMode: x2}),
      fQ: (x0,x1) => { x0.lang = x1 },
      fR: x0 => x0.width,
      g: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      gB: o => o instanceof Uint16Array,
      gC: (a, i) => a[i],
      gD: x0 => x0.hasFocus(),
      gE: x0 => x0.preventDefault(),
      gF: x0 => x0.firstChild,
      gG: (x0,x1) => x0.replaceWith(x1),
      gH: x0 => x0.value,
      gI: x0 => x0.url,
      gJ: x0 => globalThis.BigInt(x0),
      gK: (x0,x1,x2) => globalThis.Atomics.notify(x0,x1,x2),
      gL: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      gM: (x0,x1) => ({name: x0,length: x1}),
      gN: x0 => x0.tracks,
      gO: x0 => x0.height,
      gP: (x0,x1) => x0.getUserMedia(x1),
      gQ: (x0,x1) => { x0.type = x1 },
      gR: x0 => x0.remove(),
      h: (x0,x1) => ({addView: x0,removeView: x1}),
      hB: Function.prototype.call.bind(DataView.prototype.getUint16),
      hC: a => a.length,
      hD: x0 => x0.relatedTarget,
      hE: x0 => x0.parent,
      hF: x0 => x0.viewConstraints,
      hG: (x0,x1) => { x0.type = x1 },
      hH: x0 => x0.done,
      hI: x0 => x0.status,
      hJ: (x0,x1,x2) => x0.sqlite3_bind_null(x1,x2),
      hK: (x0,x1,x2) => globalThis.Atomics.store(x0,x1,x2),
      hL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2,x3) { return wasmFunction(f,arguments.length,x0,x1,x2,x3) }),
      hM: (x0,x1) => x0.update(x1),
      hN: () => globalThis.window.ImageDecoder,
      hO: x0 => x0.width,
      hP: x0 => x0.deviceId,
      hQ: (x0,x1) => { x0.defer = x1 },
      hR: (x0,x1) => { x0.src = x1 },
      i: (l, r) => l === r,
      iB: o => o instanceof Int16Array,
      iC: (x0,x1) => x0.test(x1),
      iD: x0 => x0.shiftKey,
      iE: x0 => x0.timeStamp,
      iF: x0 => x0.hostElement,
      iG: (x0,x1) => { x0.className = x1 },
      iH: (o, m, a) => o[m].apply(o, a),
      iI: x0 => x0.getReader(),
      iJ: (x0,x1) => x0.sqlite3_bind_parameter_count(x1),
      iK: x0 => new Worker(x0),
      iL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2,x3) { return wasmFunction(f,arguments.length,x0,x1,x2,x3) }),
      iM: x0 => x0.name,
      iN: x0 => x0.decode(),
      iO: x0 => x0.rasterEndMilliseconds,
      iP: x0 => x0.getCapabilities(),
      iQ: (x0,x1) => { x0.async = x1 },
      iR: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      j: x0 => x0.random(),
      jB: Function.prototype.call.bind(DataView.prototype.getInt16),
      jC: x0 => x0.userAgent,
      jD: (decoder, codeUnits) => decoder.decode(codeUnits),
      jE: (x0,x1) => x0.hasAttribute(x1),
      jF: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      jG: (x0,x1) => { x0.tabIndex = x1 },
      jH: x0 => x0.iterator,
      jI: x0 => x0.read(),
      jJ: (x0,x1) => x0.sqlite3_stmt_isexplain(x1),
      jK: () => globalThis.Uint8Array,
      jL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      jM: x0 => globalThis.IDBKeyRange.only(x0),
      jN: (x0,x1,x2,x3) => x0.open(x1,x2,x3),
      jO: x0 => x0.rasterStartMilliseconds,
      jP: () => ({}),
      jQ: (x0,x1) => { x0.id = x1 },
      jR: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      k: o => o,
      kB: o => o instanceof Uint8ClampedArray,
      kC: x0 => x0.navigator,
      kD: () => new TextDecoder("utf-8", {fatal: true}),
      kE: x0 => x0.buttons,
      kF: x0 => ({runApp: x0}),
      kG: (x0,x1) => { x0.name = x1 },
      kH: () => globalThis.Symbol,
      kI: x0 => x0.value,
      kJ: (x0,x1) => x0.dart_sqlite3_free(x1),
      kK: o => {
        if (o === null || o === undefined) return 0;
        if (o instanceof DataView) return 1;
        return 2;
      },
      kL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      kM: (x0,x1,x2) => x0.put(x1,x2),
      kN: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      kO: x0 => x0.imageBitmaps,
      kP: (x0,x1) => x0.applyConstraints(x1),
      kQ: x0 => x0.videoHeight,
      kR: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      l: o => {
        if (o === undefined || o === null) return 0;
        if (typeof o === 'number') return 1;
        return 2;
      },
      lB: o => {
        if (o === null || o === undefined) return 0;
        if (o instanceof Uint8Array) return 1;
        return 2;
      },
      lC: Function.prototype.call.bind(String.prototype.toLowerCase),
      lD: () => new TextDecoder("utf-8", {fatal: false}),
      lE: x0 => x0.ctrlKey,
      lF: Function.prototype.call.bind(DataView.prototype.getBigInt64),
      lG: (x0,x1) => { x0.placeholder = x1 },
      lH: (x0,x1) => new Intl.Segmenter(x0,x1),
      lI: x0 => x0.done,
      lJ: (x0,x1,x2,x3,x4,x5,x6) => x0.sqlite3_prepare_v3(x1,x2,x3,x4,x5,x6),
      lK: () => globalThis.DataView,
      lL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      lM: (x0,x1) => x0.getKey(x1),
      lN: (x0,x1,x2) => x0.addEventListener(x1,x2),
      lO: x0 => x0.canvasKitMaximumSurfaces,
      lP: (x0,x1) => { x0.whiteBalanceMode = x1 },
      lQ: x0 => x0.videoWidth,
      lR: (x0,x1) => { x0.oncancel = x1 },
      m: () => globalThis.Math,
      mB: Function.prototype.call.bind(DataView.prototype.setInt32),
      mC: Object.is,
      mD: (a, i, v) => a[i] = v,
      mE: x0 => x0.y,
      mF: Function.prototype.call.bind(DataView.prototype.setBigInt64),
      mG: (x0,x1) => { x0.autocomplete = x1 },
      mH: x0 => x0.Segmenter,
      mI: x0 => x0.cancel(),
      mJ: (x0,x1,x2,x3,x4,x5) => x0.sqlite3_exec(x1,x2,x3,x4,x5),
      mK: x0 => x0.communicationBuffer,
      mL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      mM: (x0,x1) => x0.delete(x1),
      mN: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      mO: x0 => x0.nextSibling,
      mP: x0 => x0.whiteBalanceMode,
      mQ: x0 => x0.stream,
      mR: (x0,x1) => { x0.onchange = x1 },
      n: (x0,x1) => x0.prepend(x1),
      nB: Function.prototype.call.bind(DataView.prototype.setUint32),
      nC: x0 => x0.vendor,
      nD: (jsArray, jsArrayOffset, wasmArray, wasmArrayOffset, length) => {
        const setValue = dartInstance.exports.$wasmI8ArraySet;
        for (let i = 0; i < length; i++) {
          setValue(wasmArray, wasmArrayOffset + i, jsArray[jsArrayOffset + i]);
        }
      },
      nE: x0 => x0.x,
      nF: (o, start, length) => new BigInt64Array(o.buffer, o.byteOffset + start, length),
      nG: (x0,x1) => { x0.name = x1 },
      nH: x0 => x0.buffer,
      nI: x0 => x0.body,
      nJ: (x0,x1,x2) => x0.sqlite3_column_name(x1,x2),
      nK: () => globalThis.Int32Array,
      nL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      nM: (x0,x1) => x0.put(x1),
      nN: x0 => x0.send(),
      nO: (x0,x1) => x0.debug(x1),
      nP: (x0,x1) => { x0.exposureMode = x1 },
      nQ: x0 => x0.play(),
      nR: x0 => x0.type,
      o: (x0,x1,x2,x3) => x0.addEventListener(x1,x2,x3),
      oB: Function.prototype.call.bind(DataView.prototype.setInt16),
      oC: (x0,x1) => x0.createTextNode(x1),
      oD: (jsArray, jsArrayOffset, wasmArray, wasmArrayOffset, length) => {
        const setValue = dartInstance.exports.$wasmI16ArraySet;
        for (let i = 0; i < length; i++) {
          setValue(wasmArray, wasmArrayOffset + i, jsArray[jsArrayOffset + i]);
        }
      },
      oE: x0 => x0.scrollTop,
      oF: o => o.byteLength,
      oG: (x0,x1) => { x0.placeholder = x1 },
      oH: x0 => x0.wasmMemory,
      oI: x0 => x0.headers,
      oJ: (x0,x1,x2) => x0.sqlite3_column_blob(x1,x2),
      oK: x0 => x0.byteLength,
      oL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      oM: (x0,x1,x2) => x0.postMessage(x1,x2),
      oN: x0 => x0.status,
      oO: x0 => x0.hostElement,
      oP: x0 => x0.exposureMode,
      oQ: x0 => x0.paused,
      oR: x0 => x0.lastModified,
      p: b => !!b,
      pB: Function.prototype.call.bind(DataView.prototype.setUint16),
      pC: (x0,x1) => { x0.id = x1 },
      pD: (jsArray, jsArrayOffset, wasmArray, wasmArrayOffset, length) => {
        const setValue = dartInstance.exports.$wasmI32ArraySet;
        for (let i = 0; i < length; i++) {
          setValue(wasmArray, wasmArrayOffset + i, jsArray[jsArrayOffset + i]);
        }
      },
      pE: x0 => x0.offsetTop,
      pF: (x0,x1,x2,x3) => x0.pushState(x1,x2,x3),
      pG: (x0,x1) => { x0.action = x1 },
      pH: () => globalThis.window._flutter_skwasmInstance,
      pI: x0 => x0.signal,
      pJ: (x0,x1,x2) => x0.sqlite3_column_bytes(x1,x2),
      pK: x0 => x0.synchronizationBuffer,
      pL: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      pM: x0 => x0.port2,
      pN: x0 => x0.response,
      pO: x0 => x0.location,
      pP: (x0,x1) => { x0.focusMode = x1 },
      pQ: (x0,x1,x2,x3) => x0.drawImage(x1,x2,x3),
      pR: x0 => x0.name,
      q: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      qB: Function.prototype.call.bind(DataView.prototype.setUint8),
      qC: (x0,x1) => { x0.nonce = x1 },
      qD: x0 => x0.visibilityState,
      qE: x0 => x0.scrollLeft,
      qF: x0 => x0.history,
      qG: (x0,x1) => { x0.method = x1 },
      qH: () => new TextDecoder(),
      qI: () => new MessageChannel(),
      qJ: (x0,x1,x2) => x0.sqlite3_column_text(x1,x2),
      qK: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      qL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2) { return wasmFunction(f,arguments.length,x0,x1,x2) }),
      qM: x0 => x0.terminate(),
      qN: (x0,x1,x2) => x0.setRequestHeader(x1,x2),
      qO: (x0,x1) => x0.getModifierState(x1),
      qP: x0 => x0.focusMode,
      qQ: (x0,x1,x2,x3,x4) => x0.getImageData(x1,x2,x3,x4),
      qR: (x0,x1) => x0.item(x1),
      r: (x0,x1) => x0.focus(x1),
      rB: Function.prototype.call.bind(DataView.prototype.setInt8),
      rC: x0 => x0.nonce,
      rD: (x0,x1,x2) => x0.removeEventListener(x1,x2),
      rE: x0 => x0.offsetLeft,
      rF: (x0,x1,x2,x3) => x0.replaceState(x1,x2,x3),
      rG: (x0,x1) => { x0.noValidate = x1 },
      rH: x0 => x0.debugSkipFontRetryDelay,
      rI: x0 => new BroadcastChannel(x0),
      rJ: (x0,x1,x2) => x0.sqlite3_column_double(x1,x2),
      rK: (o, p, v) => o[p] = v,
      rL: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      rM: (x0,x1) => new SharedWorker(x0,x1),
      rN: (x0,x1) => { x0.responseType = x1 },
      rO: x0 => x0.metaKey,
      rP: x0 => x0.enumerateDevices(),
      rQ: (x0,x1,x2) => x0.readBarcodes(x1,x2),
      rR: x0 => x0.length,
      s: () => ({}),
      sB: Function.prototype.call.bind(DataView.prototype.getInt8),
      sC: () => globalThis.window.flutterConfiguration,
      sD: x0 => x0.disconnect(),
      sE: x0 => x0.offsetParent,
      sF: o => {
        const proto = Object.getPrototypeOf(o);
        return proto === Object.prototype || proto === null;
      },
      sG: (x0,x1) => x0.removeAttribute(x1),
      sH: (x0,x1,x2) => x0.set(x1,x2),
      sI: (x0,x1,x2,x3) => x0.addEventListener(x1,x2,x3),
      sJ: x0 => globalThis.Number(x0),
      sK: (x0,x1,x2) => x0.postMessage(x1,x2),
      sL: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      sM: x0 => x0.start(),
      sN: () => new XMLHttpRequest(),
      sO: x0 => x0.altKey,
      sP: x0 => x0.deviceId,
      sQ: x0 => x0.text,
      sR: x0 => x0.files,
      t: (o, p, v) => o[p] = v,
      tB: o => {
        if (o === null || o === undefined) return 0;
        if (o instanceof Int8Array) return 1;
        return 2;
      },
      tC: (x0,x1) => x0.attachShadow(x1),
      tD: x0 => new Intl.Locale(x0),
      tE: (o, p, r) => o.replace(p, () => r),
      tF: o => Object.keys(o),
      tG: x0 => x0.isConnected,
      tH: x0 => x0.fontFallbackBaseUrl,
      tI: (x0,x1,x2,x3) => x0.removeEventListener(x1,x2,x3),
      tJ: (x0,x1,x2) => x0.sqlite3_column_int64(x1,x2),
      tK: x0 => new SharedArrayBuffer(x0),
      tL: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      tM: x0 => x0.port,
      tN: () => new FileReader(),
      tO: x0 => x0.ctrlKey,
      tP: x0 => x0.kind,
      tQ: x0 => x0.format,
      tR: x0 => x0.target,
      u: () => [],
      uB: (o, start, length) => new Float64Array(o.buffer, o.byteOffset + start, length),
      uC: (x0,x1) => x0.createElement(x1),
      uD: x0 => x0.region,
      uE: (o, p, r) => o.replaceAll(p, () => r),
      uF: x0 => x0.state,
      uG: x0 => x0.click(),
      uH: (handle) => clearInterval(handle),
      uI: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      uJ: (x0,x1,x2) => x0.sqlite3_column_type(x1,x2),
      uK: (x0,x1,x2,x3) => ({clientVersion: x0,root: x1,synchronizationBuffer: x2,communicationBuffer: x3}),
      uL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2,x3,x4) { return wasmFunction(f,arguments.length,x0,x1,x2,x3,x4) }),
      uM: (x0,x1) => x0.sqlite3_last_insert_rowid(x1),
      uN: (x0,x1) => x0.readAsArrayBuffer(x1),
      uO: x0 => x0.isComposing,
      uP: x0 => x0.mediaDevices,
      uQ: x0 => x0.bytes,
      uR: (x0,x1) => x0.replaceChildren(x1),
      v: (a, i) => a.push(i),
      vB: (o, start, length) => new Float32Array(o.buffer, o.byteOffset + start, length),
      vC: x0 => x0.scale,
      vD: x0 => x0.script,
      vE: x0 => x0.deltaMode,
      vF: x0 => x0.state,
      vG: (x0,x1) => x0.getElementsByClassName(x1),
      vH: (ms, c) =>
      setInterval(() => dartInstance.exports.$invokeCallback(c), ms),
      vI: x0 => globalThis.Array.isArray(x0),
      vJ: (x0,x1) => x0.sqlite3_column_count(x1),
      vK: x0 => x0.close(),
      vL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2,x3) { return wasmFunction(f,arguments.length,x0,x1,x2,x3) }),
      vM: x0 => x0.abort(),
      vN: x0 => x0.result,
      vO: x0 => x0.code,
      vP: x0 => x0.facingMode,
      vQ: x0 => x0.y,
      vR: x0 => x0.click(),
      w: x0 => new Int8Array(x0),
      wB: (o, start, length) => new Uint32Array(o.buffer, o.byteOffset + start, length),
      wC: x0 => x0.visualViewport,
      wD: x0 => x0.language,
      wE: x0 => x0.deltaY,
      wF: (x0,x1) => x0.go(x1),
      wG: (jsArray, jsArrayOffset, wasmArray, wasmArrayOffset, length) => {
        const setValue = dartInstance.exports.$wasmF32ArraySet;
        for (let i = 0; i < length; i++) {
          setValue(wasmArray, wasmArrayOffset + i, jsArray[jsArrayOffset + i]);
        }
      },
      wH: () => Date.now(),
      wI: x0 => x0.table,
      wJ: (x0,x1) => x0.sqlite3_close_v2(x1),
      wK: x0 => x0.getSize(),
      wL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2,x3) { return wasmFunction(f,arguments.length,x0,x1,x2,x3) }),
      wM: x0 => x0.naturalHeight,
      wN: () => new XMLHttpRequest(),
      wO: x0 => x0.repeat,
      wP: x0 => x0.mediaDevices,
      wQ: x0 => x0.x,
      wR: (x0,x1,x2) => x0.setAttribute(x1,x2),
      x: (jsArray, jsArrayOffset, wasmArray, wasmArrayOffset, length) => {
        const getValue = dartInstance.exports.$wasmI8ArrayGet;
        for (let i = 0; i < length; i++) {
          jsArray[jsArrayOffset + i] = getValue(wasmArray, wasmArrayOffset + i);
        }
      },
      xB: (o, start, length) => new Int32Array(o.buffer, o.byteOffset + start, length),
      xC: x0 => x0.devicePixelRatio,
      xD: x0 => x0.languages,
      xE: x0 => x0.deltaX,
      xF: x0 => x0.hash,
      xG: (jsArray, jsArrayOffset, wasmArray, wasmArrayOffset, length) => {
        const setValue = dartInstance.exports.$wasmF64ArraySet;
        for (let i = 0; i < length; i++) {
          setValue(wasmArray, wasmArrayOffset + i, jsArray[jsArrayOffset + i]);
        }
      },
      xH: (x0,x1,x2) => x0.insertBefore(x1,x2),
      xI: x0 => x0.kind,
      xJ: (x0,x1,x2,x3,x4,x5,x6) => x0.dart_sqlite3_create_function_v2(x1,x2,x3,x4,x5,x6),
      xK: (x0,x1) => x0.truncate(x1),
      xL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1,x2,x3) { return wasmFunction(f,arguments.length,x0,x1,x2,x3) }),
      xM: x0 => x0.naturalWidth,
      xN: (x0,x1,x2,x3) => x0.open(x1,x2,x3),
      xO: (wasmFunction,f) => finalizeWrapper(f, function(x0) { return wasmFunction(f,arguments.length,x0) }),
      xP: () => globalThis.BarcodeDetector.getSupportedFormats(),
      xQ: x0 => x0.bottomLeft,
      xR: (x0,x1) => { x0.accept = x1 },
      y: x0 => new Uint8Array(x0),
      yB: (o, start, length) => new Uint16Array(o.buffer, o.byteOffset + start, length),
      yC: x0 => x0.height,
      yD: (x0,x1) => x0.observe(x1),
      yE: x0 => x0.wheelDeltaY,
      yF: x0 => x0.location,
      yG: (x0,x1) => x0.dispatchEvent(x1),
      yH: x0 => x0.id,
      yI: x0 => x0.data,
      yJ: (x0,x1,x2,x3) => x0.sqlite3_result_error(x1,x2,x3),
      yK: x0 => ({at: x0}),
      yL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      yM: (x0,x1) => x0.createElement(x1),
      yN: x0 => x0.send(),
      yO: x0 => x0.userAgent,
      yP: (x0,x1) => x0.call(x1),
      yQ: x0 => x0.bottomRight,
      yR: (x0,x1) => { x0.multiple = x1 },
      z: x0 => new Uint8ClampedArray(x0),
      zB: (o, start, length) => new Int16Array(o.buffer, o.byteOffset + start, length),
      zC: x0 => x0.width,
      zD: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      zE: x0 => x0.wheelDeltaX,
      zF: x0 => x0.search,
      zG: (x0,x1) => x0.createEvent(x1),
      zH: x0 => x0.offsetHeight,
      zI: x0 => x0.close(),
      zJ: (x0,x1,x2) => x0.sqlite3_result_subtype(x1,x2),
      zK: (x0,x1) => x0.write(x1),
      zL: (wasmFunction,f) => finalizeWrapper(f, function(x0,x1) { return wasmFunction(f,arguments.length,x0,x1) }),
      zM: (x0,x1) => { x0.pointerEvents = x1 },
      zN: x0 => x0.type,
      zO: (x0,x1,x2,x3) => x0.open(x1,x2,x3),
      zP: x0 => x0.reset,
      zQ: x0 => x0.topRight,
      zR: (x0,x1) => { x0.type = x1 },

    };

    const baseImports = {
      _: dart2wasm,
      Math: Math,
      Date: Date,
      Object: Object,
      Array: Array,
      Reflect: Reflect,
      WebAssembly: {
        JSTag: WebAssembly.JSTag,
      },
      "": new Proxy({}, { get(_, prop) { return prop; } }),

    };

    const jsStringPolyfill = {
      "charCodeAt": (s, i) => s.charCodeAt(i),
      "compare": (s1, s2) => {
        if (s1 < s2) return -1;
        if (s1 > s2) return 1;
        return 0;
      },
      "concat": (s1, s2) => s1 + s2,
      "equals": (s1, s2) => s1 === s2,
      "fromCharCode": (i) => String.fromCharCode(i),
      "length": (s) => s.length,
      "substring": (s, a, b) => s.substring(a, b),
      "fromCharCodeArray": (a, start, end) => {
        if (end <= start) return '';

        const read = dartInstance.exports.$wasmI16ArrayGet;
        let result = '';
        let index = start;
        const chunkLength = Math.min(end - index, 500);
        let array = new Array(chunkLength);
        while (index < end) {
          const newChunkLength = Math.min(end - index, 500);
          for (let i = 0; i < newChunkLength; i++) {
            array[i] = read(a, index++);
          }
          if (newChunkLength < chunkLength) {
            array = array.slice(0, newChunkLength);
          }
          result += String.fromCharCode(...array);
        }
        return result;
      },
      "intoCharCodeArray": (s, a, start) => {
        if (s === '') return 0;

        const write = dartInstance.exports.$wasmI16ArraySet;
        for (var i = 0; i < s.length; ++i) {
          write(a, start++, s.charCodeAt(i));
        }
        return s.length;
      },
      "test": (s) => typeof s == "string",
    };


    

    dartInstance = await WebAssembly.instantiate(this.module, {
      ...baseImports,
      ...additionalImports,
      
      "wasm:js-string": jsStringPolyfill,
    });

    return new InstantiatedApp(this, dartInstance);
  }
}

class InstantiatedApp {
  constructor(compiledApp, instantiatedModule) {
    this.compiledApp = compiledApp;
    this.instantiatedModule = instantiatedModule;
  }

  // Call the main function with the given arguments.
  invokeMain(...args) {
    this.instantiatedModule.exports.$invokeMain(args);
  }
}
